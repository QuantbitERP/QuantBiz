...
class LeaveViewModel extends BaseViewModel {
  ...
  Future<List<LeaveList>> fetchupcomingleaveForCurrentYear() async {
    ...
    List<LeaveList> filteredLeaves = holidays.where((leave) {
      DateTime? postingDate = DateTime.parse(
          leave.postingDate ?? ""); // Assuming postingDate is a String
      Logger().i(leave.postingDate);
      return postingDate.year == _selectedYear &&
          postingDate.month == _selectedmonth;
    }).toList();
...
// Set the color for Closed status
  ...
  String getMonthName(int month) {
    ...
    DateTime dateTime = DateTime(2023, month,
        1); // You can set any year, as it won't affect the month name
